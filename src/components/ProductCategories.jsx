...
const ProductCategories = () => {
  ...
  return (
    <div>
      <div className="container mx-auto px-2 sm:px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 sm:gap-6 gap-2">
        {categoriesLoading
          ? [...Array(3)].map((_, index) => (
              <section
                key={index}
                className="bg-white p-5 rounded-sm shadow-md animate-pulse"
              >
          ...
          : categories.map((category) => (
              <section
                key={category}
                className="bg-white p-5 rounded-sm shadow-md"
              >
                ...
                <Link
                  ...
                  className="text-blue-500 mt-3 block text-sm font-semibold"
                >
                  See more
                </Link>
              </section>
      ...
      {categoriesError && <p>Error in categories: {categoriesError}</p>}
      {productsError && <p>Error in products: {productsError}</p>}
    </div>
  );
};
